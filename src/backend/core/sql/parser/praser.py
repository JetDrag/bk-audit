...
class SqlQueryAnalysis:
    ...
    def parse_sql(self):
        ...
        for var_node in chain(
            ...
            if "?" == raw_name:
                ...
                raise SQLParseError(f"重复的变量名 '{raw_name}'")
        ...
        elif isinstance(current_expr, exp.Union):
    ...
    def generate_sql_with_values(
        self, params: dict, sql_template: Optional[str] = None, template_dialect: Optional[str] = None
    ) -> str:
        """
        使用 sqlglot 将 SQL 模板中的命名参数替换为实际值，生成完整的 SQL 语句。
        """
        ...
        return transformed_tree.sql(dialect=target_dialect)
