...
class TestSqlQueryAnalysis(TestCase):
    ...
    def test_generate_sql_with_values(self):
        ...
        assert "category = 'electronics'" in generated_sql
        assert "price > 1000" in generated_sql
    ...
    def test_duplicate_variable_names(self):
        """测试重复变量名"""
        sql = "SELECT * FROM users WHERE id = :user_id OR name = :user_id"
        analyzer = SqlQueryAnalysis(sql)
        with self.assertRaises(SQLParseError):
            analyzer.parse_sql()
    ...
    def test_external_template_generation(self):
        ...
        generated = analyzer.generate_sql_with_values(params, sql_template=template)
